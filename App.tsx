...
const App: React.FC = () => {
  ...
  const handleProcess = useCallback(async () => {
    ...
    try {
      ...
      if (err.message.includes('API key') || err.message.includes('API Anahtarı')) {
        setError({
            message: "Yetkilendirme Hatası",
            details: "API anahtarınız geçersiz olabilir. Lütfen anahtar simgesine tıklayarak kontrol edin."
        });
      } else {
        setError({
